 Meteor.methods({
   ...
   submitPreferences: function(preference){
     if(Preferences.find({user:Meteor.userId()}).count()==0){
       console.dir("Insert new Preference");
       Preferences.insert(preference);
     }else{
       console.dir("Prefernces updated");
       preference.user = this.userId;
       Preferences.remove({user: this.userId});
       Preferences.insert(preference);
       console.dir("Here!");
     }
   },

   insertMeals: function(meals){
     Meals.insert(meals);
   },
 })
